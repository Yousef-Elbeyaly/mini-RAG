...
class PGVectorProvider(VectorDBInterface):
    ...
    def __init__(self, db_client, default_vector_size: int = 700,
    ...
    async def create_vector_index(self, collection_name: str,
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                result = await session.execute(create_idx_sql)
    ...
    async def insert_one (self, collection_name: str,
        ...
        if not record_id:
            self.logger.info(f"Can not insert a new record without chunk_id: {collection_name}")
            return False
