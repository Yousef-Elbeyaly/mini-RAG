        ...
        while has_records:
            ...
            if not is_inserted:
                task_instance.update_state(
                    state="FAILURE",
                    meta={
                        "signal": ResponseSignal.INSERT_INTO_VEXTORDB_ERROR.value
                    }
                )
            ...
            inseted_items_count += len(page_chunks)


        task_instance.update_state(
                state="SUCCESS",
                meta={
                    "signal": ResponseSignal.INSERT_INTO_VEXTORDB_SUCCESS.value
                }
            )
        
        return {
                "signal": ResponseSignal.INSERT_INTO_VEXTORDB_SUCCESS.value,
                "inserted_items_count": inseted_items_count
            }
